...
const EventActions = (props) => {
  ...
  async function printSlip() {
    ...
    await handler
      .get(
        `${API_BASE_URL}/pdfgen/?template_type=slip&id=` + activeIncident.id,
        {
          responseType: "blob",
        }
      )
    ...
    <div className={classes.card}>
      ...
      <List className={classes.root}>
        ...
        >
          ...
          <ListItemText
            primary="Assigned to"
            secondary={
              activeIncident.assignee ? activeIncident.assignee.displayname : ""
            }
          />
          ...
          {activeIncident.currentStatus !== "CLOSED" &&
            ...
            userCan(
              currentUser,
              activeIncident,
              USER_ACTIONS.CAN_CHANGE_ASSIGNEE
            ) && (
              <ListItemSecondaryAction>
                <IconButton
                  aria-label="Edit"
                  onClick={() => {
                    props.modalAction("CHANGE_ASSIGNEE_MODAL");
                  }}
                >
                  <EditIcon />
                </IconButton>
        ...
        <ListItem>
          ...
          <ListItemText
            primary="Last actioned at"
            secondary={getLastActionTime(props.events)}
          />
        </ListItem>
        ...
        <ListItem>
          ...
          <ListItemText
            primary="Close within"
            secondary={activeIncident.response_time + " hours."}
          />
          {activeIncident.currentStatus !== "CLOSED" && (
            <ListItemSecondaryAction>
              <IconButton
                aria-label="Edit"
                onClick={() => {
                  props.modalAction("RESPONSE_TIME_EDIT");
                }}
              >
                <EditIcon />
              </IconButton>
        ...
        <ListItem>
          ...
          <ListItemText
            ...
            classes={{
              // inset:true,
              secondary:
                calculateDeadline(activeIncident).status === "OVERDUE" &&
          ...
          <>
            ...
            {(activeIncident.currentStatus === "NEW" ||
              ...
              userCan(
                currentUser,
                activeIncident,
                USER_ACTIONS.CAN_INVALIDATE_INCIDENT
              ) && (
                <Button
                  color="primary"
                  size="large"
                  ...
                  className={classes.button}
                  onClick={() => {
                    dispatch(showModal("INVALIDATE_MODAL", { activeIncident }));
                  }}
                >
      ...
      {activeIncident.currentStatus === "CLOSED" &&
        userCan(
          currentUser,
          activeIncident,
          USER_ACTIONS.CAN_REOPEN_INCIDENT
        ) && (
          <Button
            color="primary"
            size="large"
            ...
            className={classes.button}
            onClick={() => {
              dispatch(showModal("REOPEN_MODAL", { activeIncident }));
            }}
          >
      ...
      {activeIncident.currentStatus != "CLOSED" &&
        activeIncident.incidentType === "INQUIRY" && (
